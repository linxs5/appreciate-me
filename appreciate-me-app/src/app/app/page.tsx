...
export default function GaragePage() {
  ...
  useEffect(() => {
    getVehicles().then(v => { setVehicles(v); setLoading(false) })
  }, [])
    ...
    <div style={{ minHeight: '100vh', background: 'var(--black)', padding: '0' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div className="fade-up" style={{ marginBottom: 32 }}>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: 'var(--accent)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8 }}>— GARAGE</div>
          <h1 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(36px,6vw,60px)', color: 'var(--off-white)', lineHeight: 1, letterSpacing: '0.02em' }}>YOUR GARAGE</h1>
          <p style={{ color: 'var(--gray)', fontSize: 14, marginTop: 6 }}>{vehicles.length} vehicle{vehicles.length !== 1 ? 's' : ''} tracked</p>
        </div>
          ...
          <div className="fade-up delay-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 40 }}>
            {[
              ...
              <div key={i} style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 6, padding: '16px 20px' }}>
                <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: 'var(--gray)', letterSpacing: '0.12em', marginBottom: 8 }}>{s.label}</div>
                <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 32, color: 'var(--off-white)', lineHeight: 1 }}>{s.value}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Vehicles grid */}
        {loading ? (
          <div style={{ color: 'var(--gray)', fontFamily: 'DM Mono, monospace', fontSize: 13, padding: '60px 0', textAlign: 'center' }}>LOADING GARAGE...</div>
        ) : vehicles.length === 0 ? (
          <div className="fade-in" style={{ textAlign: 'center', padding: '80px 0' }}>
            <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 32, color: 'var(--gray)', marginBottom: 16 }}>NO VEHICLES YET</div>
            <p style={{ color: 'var(--gray)', marginBottom: 24, fontSize: 15 }}>Add your first vehicle to start building your documented history.</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px,1fr))', gap: 16 }}>
            {vehicles.map((v, i) => (
              <Link key={v.id} href={`/app/vehicles/${v.id}`} style={{ textDecoration: 'none' }}
                className={`fade-up delay-${Math.min(i + 1, 6)}`}>
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden', transition: 'border-color 0.2s, transform 0.2s' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(200,240,0,0.3)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(0)' }}>
                  {/* Photo */}
                  <div style={{ height: 180, background: '#1a1a18', overflow: 'hidden' }}>
                    {v.photoKeys?.[0] ? (
                      <img src={photoUrl(v.photoKeys[0])} alt={`${v.year} ${v.make} ${v.model}`} className="card-photo" loading="lazy" />
                    ) : (
                  ...
                  <div style={{ padding: '16px 18px' }}>
                    ...
                    {v.trim && <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 11, color: 'var(--gray)', marginBottom: 12 }}>{v.trim}</div>}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 0' }}>
                      {[
                        ...
                        <div key={j}>
                          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 9, color: 'var(--gray)', letterSpacing: '0.1em' }}>{s.l}</div>
                          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 13, color: 'var(--off-white)' }}>{s.v}</div>
                        </div>
                      ))}
                    </div>
                  </div>
