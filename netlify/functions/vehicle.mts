...
export default async (req: Request) => {
  ...
  if (req.method === 'PUT') {
    ...
    if (body.action === 'add-entry' && body.entry) {
      updated.entries = [
        { id: crypto.randomUUID(), ...body.entry, photoKeys: body.entry.photoKeys || [] },
        ...(existing.entries || []),
      ]
    } else if (body.action === 'update-entry' && body.entryId && body.entry) {
      updated.entries = (existing.entries || []).map((e: any) =>
        e.id === body.entryId ? { ...e, ...body.entry, id: e.id, photoKeys: e.photoKeys || [] } : e
      )
  ...
  updated = {
    ...
    coverPhotoKey:
      ...
      const { action, ...patch } = body
      updated = { ...existing, ...patch, id: existing.id, createdAt: existing.createdAt }
    }
